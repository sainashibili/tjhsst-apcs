// Name: Saina Shibili   Date: 01/27/2021
...
public class LunchRoom
{
    ...
    ******************/ 
      ...
      public Customer(String g, int a, int o){
         gradYear = g;
         arrivedAt = a;
         orderAndBeServed = o;
      }
      ...
      public void setGradYear(String g){
         gradYear = g;
      }
      ...
      public int compareTo(Customer c){
         if(gradYear.compareTo(c.getGradYear()) != 0)
            return gradYear.compareTo(c.getGradYear());
         else if(arrivedAt > c.getArrivedAt())
            return 1;
         else
            return -1;   
      }
   ...
   public static void LunchRoom()
   {
      ...
      *************************/
      ...
      String[] classes = {"Senior", "Junior", "Sophomore", "Freshman"};
      int[] served = {0, 0, 0, 0};
      int[] lWait = {0, 0, 0, 0};
      int[] aWait = {0, 0, 0, 0};
      ...
      for(int i = 0; i < TIME; i++){
         if(Math.random() <= CHANCE_OF_CUSTOMER){
            String s = classOf();
            q.add(new Customer(s, i, timeToOrderAndBeServed()));
            switch(s){
               case "2021":
                  served[0]++;
               case "2022":
         ...
         if(!q.isEmpty()){
            if(q.peek().getOrderAndBeServed() > 0)
               ...
               int thisCustomersTime = i - q.peek().getArrivedAt();
               switch(q.poll().getGradYear()){
                  case "2021":
                     aWait[0] += thisCustomersTime;
                     if(thisCustomersTime > lWait[0])
                        lWait[0] = thisCustomersTime;
                  case "2022":
                     aWait[1] += thisCustomersTime;
                     if(thisCustomersTime > lWait[1])
                        lWait[1] = thisCustomersTime;
                  case "2023":
                     aWait[2] += thisCustomersTime;
                     if(thisCustomersTime > lWait[2])
                        lWait[2] = thisCustomersTime;
                  case "2024":
                     aWait[3] += thisCustomersTime;
                     if(thisCustomersTime > lWait[3])
                        lWait[3] = thisCustomersTime;
               }
            }
